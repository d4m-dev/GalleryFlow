...
export default function ExplorePage() {
  ...
  if (loading) return <div className="flex justify-center py-20 min-h-screen dark:bg-stone-950"><Loader2 className="animate-spin text-amber-500 w-12 h-12" /></div>;

  return (
    <div className="max-w-7xl mx-auto px-4 py-16 pb-32">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
        <div>
          <h1 className="text-6xl font-black text-stone-900 dark:text-white tracking-tighter italic uppercase">Khám phá</h1>
          <p className="text-stone-500 dark:text-stone-400 mt-4 text-lg">Cảm hứng từ cộng đồng sáng tạo.</p>
        </div>
        <div className="relative group w-full md:w-96">
          ...
          <input 
            ...
            className="w-full pl-12 pr-6 py-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl outline-none focus:ring-2 focus:ring-amber-500/20 dark:text-white shadow-sm" 
          />
        </div>
      </div>
      ...
      <div className="columns-2 md:columns-3 lg:columns-4 gap-6 space-y-6">
        {filtered.map((photo, index) => (
          <div key={photo.id} className="relative group break-inside-avoid rounded-[2.5rem] overflow-hidden bg-stone-100 dark:bg-stone-800 shadow-sm hover:shadow-2xl transition-all duration-700">
            ...
            <div className="absolute inset-0 pointer-events-none p-8 flex flex-col justify-between">
              ...
              <div className="opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0">
                ...
                <p className="text-amber-400 text-[10px] uppercase font-black tracking-widest mt-2">{photo.albums?.title}</p>
              </div>
            </div>
          </div>
        ))}

    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="aspect-[4/3] bg-stone-200 rounded-2xl animate-pulse" />
          ))}
        </div>
      </div>
    );
    ...
    <div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-32">
          <div className="max-w-2xl">
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight leading-[1.1]">
              Khoảnh khắc đẹp,<br />
              <span className="text-amber-400">Trình bày tinh tế</span>
            </h1>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-10">
          <div>
            <h2 className="text-2xl font-bold text-stone-900">Bộ sưu tập</h2>
            <p className="text-stone-500 mt-1">{albums.length} album ảnh</p>
          </div>
        </div>

        {albums.length === 0 ? (
          <div className="text-center py-20">
            <Images className="w-12 h-12 text-stone-300 mx-auto mb-4" />
            <p className="text-stone-400 text-lg">Chưa có album nào được xuất bản.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {albums.map((album) => (
              ...
              >
                ...
                <div className="mt-3.5 px-1">
                  <h3 className="font-semibold text-stone-900 group-hover:text-amber-700 transition-colors">{album.title}</h3>
                  ...
                  {album.description && (
                    <p className="mt-1.5 text-sm text-stone-500 line-clamp-2">{album.description}</p>
                  )}
                </div>
              </button>
            ))}

...
export default function SuperAdmin() {
    ...
    if (activeTab === 'users') {
      ...
      if (albums && profiles) {
        const albumsWithProfiles = albums.map(album => ({
          ...album,
          profiles: profiles.find(p => p.id === album.created_by)
        }));
  ...
  async function handleBanToggle(userId: string, currentStatus: boolean) {
    if (!confirm(`Xác nhận thay đổi trạng thái người dùng?`)) return;
  ...
  async function handleDeleteAlbum(albumId: string) {
    if (!confirm("Xóa album này và toàn bộ ảnh?")) return;
  ...
  if (!isAdmin) return <div className="p-20 text-center text-stone-500">Truy cập bị từ chối.</div>;
    ...
    <div className="max-w-7xl mx-auto px-4 py-10 pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-stone-500 mt-1">Hệ thống quản lý người dùng và nội dung</p>
        </div>
      ...
      <div className="mb-8">
        {activeTab === 'users' ? (
          <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm inline-flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-stone-400 uppercase tracking-wider">Tổng người dùng</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p className="text-xs font-bold text-stone-400 uppercase tracking-wider">Tổng Album</p>
            ...
            <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p className="text-xs font-bold text-stone-400 uppercase tracking-wider">Tổng số ảnh</p>
            ...
            <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p className="text-xs font-bold text-stone-400 uppercase tracking-wider">Tổng yêu thích</p>
            ...
            <tbody className="divide-y divide-stone-100">
              {data.map((u) => (
                <tr key={u.id} className={u.is_banned ? 'bg-red-50/30' : ''}>
                  ...
                  <td className="px-6 py-4">
                    <button onClick={() => handleRoleToggle(u.id, u.role)} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold uppercase border transition-all ${u.role === 'admin' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-white border-stone-200 text-stone-500'}`}>
                      <ShieldCheck className="w-3 h-3"/> {u.role}
                    </button>
                  </td>
                  <td className="px-6 py-4">
                    {u.is_banned ? <span className="text-red-600 text-xs font-bold italic">Đã khóa</span> : <span className="text-emerald-600 text-xs font-bold">Hoạt động</span>}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleBanToggle(u.id, u.is_banned)} className={`p-2 rounded-lg transition-colors ${u.is_banned ? 'text-emerald-600 hover:bg-emerald-50' : 'text-red-400 hover:bg-red-50'}`}>
          ...
          {data.map(album => (
            <div key={album.id} className="bg-white border border-stone-200 p-4 rounded-2xl flex items-center gap-4 hover:shadow-md transition-all">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-stone-500 mt-0.5 font-medium">Bởi: {album.profiles?.email || 'N/A'}</p>
              ...
              <div className="flex gap-2">
                <button onClick={() => setViewingAlbumId(album.id)} className="p-2.5 bg-red-50 text-red-500 hover:bg-red-100 rounded-xl transition-colors" title="Xem ảnh"><ExternalLink className="w-5 h-5"/></button>
